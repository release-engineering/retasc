...
def test_run_rule_jira_search_once_per_prerequisite(factory, mock_jira, mock_pp):
    ...
    assert mock_jira.search_issues.mock_calls == [
        call(
            ...
            fields=["labels", "resolution", "status", "summary"],
        )
        for release in releases
        for _ in range(2)
    ]
...
@mark.parametrize(
    ...
    (
        ...
        ("start_date + 1|weeks", False, True),
        ("start_date - 1|week", False, True),
        ("start_date + 1|week", False, True),
        ("end_date - 1|weeks", False, True),
        ("end_date + 1|weeks", False, False),
        ("end_date - 1|week", False, True),
        ("end_date + 1|week", False, False),
        ("today", False, True),
        ("today", True, False),
        ("today + 1|days", False, False),
        ("today + 1|weeks", False, False),
        ("today + 1|day", False, False),
        ("today + 1|week", False, False),
    ),
)
...
def test_run_rule_fixed_date(factory):
    inputs = [Variables(variables={"target_date": "2020-01-01"})]
    prereq = PrerequisiteTargetDate(target_date="date(target_date) + 1|day")
    rule = factory.new_rule(inputs=inputs, prerequisites=[prereq])
    report = call_run()
    assert report.data == {
        "inputs": [
            {
