...
def test_release_schedules_with_default(pp_api, requests_mock):
    ...
    requests_mock.get(pp_api.api_url_release_schedule_tasks(TEST_RELEASE), json=test_res)
...
def test_release_schedules_by_search_query(pp_api, requests_mock):
    ...
    requests_mock.get(pp_api.api_url_release_schedule_tasks(TEST_RELEASE), json=test_res)
    resp = pp_api.release_schedules(TEST_RELEASE, **{"flags_or__in": "rcm,releng,exd,sp"})
    req = requests_mock.request_history[0]
    ...
    requests_mock.reset()
    resp = pp_api.release_schedules(TEST_RELEASE,  **{"flags_and__in": "sp,ga", "fields":"name"})
    req = requests_mock.request_history[0]
...
def test_active_release_schedules_for_product(pp_api, requests_mock):
    ...
    requests_mock.get(f"{pp_api.api_url_releases()}{releases_res[0]["shortname"]}/schedule-tasks", json=tasks_res)
    requests_mock.get(f"{pp_api.api_url_releases()}{releases_res[1]["shortname"]}/schedule-tasks", json=tasks_res)
