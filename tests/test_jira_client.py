...
def test_create_issue(jira_api, requests_mock):
    requests_mock.post(f"{JIRA_URL}/rest/api/2/issue?updateHistory=false", json=TEST_RES)
...
def test_edit_issue(jira_api, requests_mock):
    requests_mock.put(f"{JIRA_URL}/rest/api/2/issue/{ISSUE_KEY}?notifyUsers=True", json=TEST_RES)
...
def test_search_issue(jira_api, requests_mock):
    requests_mock.get(f"{JIRA_URL}/rest/api/2/search?startAt=0&fields=%2Aall&jql=project+%3D+{PROJECT_KEY}", json=SEARCH_LIST)
